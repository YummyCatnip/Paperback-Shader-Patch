...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    float adjusted_dissolve = (dissolve*dissolve*(3.0-2.*dissolve))*1.02 - 0.01; //Adjusting 0.0-1.0 to fall to -0.1 - 1.1 scale so the mask does not pause at extreme values
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    //Glow effect
    number glow = 0.0;
    int glow_samples = 4.0;
    int actual_glow_samples = 0.0;
    ...
    if (tex.a > 0.999){tex = vec4(0.0,0.0,0.0,0.0);}
    // if (tex.a < 0.001){tex.rgb = vec3(0.0,1.0,1.0);}
    vec2 uv = (((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;

    if (uv.x >0.99 || uv.x < 0.01 || uv.y > 0.99 || uv.y < 0.01){
        return vec4(0.0,0.0,0.0,0.0); // crop to get rid of blue borders
    }

    number light_strength = 0.4*(0.3*sin(2.0*hologramv2.g) + 0.6 + 0.3*sin(hologramv2.r*3.0) + 0.9);
    vec4 final_col;
    if (tex.a < 0.001){
        final_col = tex*colour + vec4(0.0, 1.0, .5,0.6)*light_strength*(1.0+abs(offset_l)+abs(offset_r))*glow;
    }
    else{
        final_col = tex*colour + vec4(0.0, 0.3, 0.2,0.3)*light_strength*(1.0+abs(offset_l)+abs(offset_r))*glow;
    }
